...
class DominoClassifier:
    ...
    def classify_domino(self, domino: np.ndarray) -> Tuple[int, int]:
        ...
        if domino.shape[0] < domino.shape[1]:
            ...
            # cv.line(img_circle, (domino.shape[1]//2, 0), (domino.shape[1]//2, domino.shape[0]), (0, 255, 0), 2)
...
if __name__ == "__main__":
    classifier = DominoClassifier()
    for file in os.listdir("../../extracted_domino_hq_clean"):
        if file.endswith(".jpg"):
            domino = cv.imread(f"../../extracted_domino_hq_clean/{file}")
            img, (cnt_0, cnt_1) = classifier.classify_domino(domino)
            cv.imwrite(f"../../draw_lines/{file}", img)